...
get_latest_version() {
    ...
    if [[ `getconf WORD_BIT` == "32" && `getconf LONG_BIT` == "64" ]]; then
        deb_name=$(wget -qO- http://kernel.ubuntu.com/~kernel-ppa/mainline/v${latest_version}/ | grep "linux-
        " | grep "generic" | awk -F'\">' '/amd64.deb/{print $2}' | cut -d'<' -f1 | head -1)
...
echo "----------------------------------------"
echo "If above information incorrect Press Ctrl + C now to cancel"
sleep 5

install_bbr

echo "Info: The system needs to be restart"